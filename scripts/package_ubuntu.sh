...
Options:
  ...
  --rebuild-webview    Force rebuild crates/app/webview-ui/dist with npm.
...
ensure_webview_dist() {
  ...
  command -v npm >/dev/null 2>&1 || die "npm not found; rerun without --skip-system-deps or install nodejs/npm first"
