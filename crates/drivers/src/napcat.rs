...
fn normalize_agent_command_block(block: &AgentCommandBlock) -> AgentCommandBlock {
    match block {
        ...
        AgentCommandBlock::ExecuteGlobalAction { action } => AgentCommandBlock::ExecuteGlobalAction {
...
fn normalize_agent_command_review_action(
    ...
    match action {
        ...
        AgentCommandReviewAction::Merge { target_review_code } => {
            AgentCommandReviewAction::Merge {
...
fn validate_agent_command_review_action(
    ...
    match action {
        ...
        AgentCommandReviewAction::Defer { delay_ms } => validate_agent_command_required_field(
            command_name,
            index,
            "延期毫秒数",
            delay_ms,
...
fn validate_agent_command_global_action(
    ...
    match action {
        ...
        AgentCommandGlobalAction::QuickReplyAdd {
            ...
            validate_agent_command_required_field(command_name, index, "快捷回复键", key_template)?;
            validate_agent_command_required_field(command_name, index, "快捷回复内容", text_template)
        }
        AgentCommandGlobalAction::QuickReplyDelete { key_template } => {
            validate_agent_command_required_field(command_name, index, "快捷回复键", key_template)
        }
...
fn map_tag_value_for_group(settings: &UserNotificationSettings, group_name: &str, raw: &str) -> String {
    let source = raw.trim().to_string();
    if source.is_empty() {
        return String::new();
    }
    let normalized_group = group_name.trim().trim_start_matches('<').trim_end_matches('>').trim();
...
fn build_user_notification_message(
    ...
    let tag_prefix = tags
        ...
        .collect::<Vec<_>>()
        .join(" ");
    let rendered_text = render_user_notification_template(&template.text_template, context, settings);
...
async fn build_agent_command_context(
    ...
    let (
        ...
        let previous_post_created_at = if previous_post_created_timestamp_ms > 0 {
            format_local_datetime(previous_post_created_timestamp_ms, runtime.tz_offset_minutes)
        } else {
            String::new()
        };
        (
...
fn resolve_agent_post_id_by_code(
    ...
    let Some((post_id, post_group)) = state
        .post_external_code
        .iter()
        .find_map(|(post_id, code)| {
...
fn build_agent_review_action(
    ...
    Ok(match action {
        ...
        AgentCommandReviewAction::Comment { text_template } => ReviewAction::Comment {
            text: render_agent_command_template(text_template, context).trim().to_string(),
        },
...
async fn execute_agent_insert_queued_post(
    ...
    let events = {
        ...
        let Some(moving_plan) = guard.send_plans.get(&moving_post_id).cloned() else {
            return Err(format!("稿件 #{} 当前不在发送队列中", moving_post_code.trim()));
        };
        let Some(anchor_plan) = guard.send_plans.get(&anchor_post_id).cloned() else {
            return Err(format!("稿件 #{} 当前不在发送队列中", anchor_post_code.trim()));
        };
        ...
        let mut queue = guard
            ...
            .collect::<Vec<_>>();
        queue.sort_by(|a, b| {
            (a.1.not_before_ms, a.1.priority, a.1.seq, a.0.0)
                .cmp(&(b.1.not_before_ms, b.1.priority, b.1.seq, b.0.0))
        });
        ...
        let Some(anchor_index) = queue.iter().position(|(post_id, _)| *post_id == anchor_post_id)
        else {
            return Err(format!("稿件 #{} 当前不在发送队列中", anchor_post_code.trim()));
        };
        ...
        queue.insert(insert_index.min(queue.len()), (moving_entry.0, updated_plan));
...
async fn execute_private_agent_command(
    ...
    for block in command.blocks {
        ...
        match block {
            ...
            AgentCommandBlock::ExecuteReviewAction { review_code, action } => {
